...
def show(batch: t.Dict[str, t.Any],
         keys: t.Tuple[str] = ('images', 'dose', 'oar', 'ptv')
         ) -> None:
...
def get_extractor() -> extr.Extractor:
    categories = ('images', 'dose', 'mask', 'oar', 'ptv')

    extractors = [
        extr.DataExtractor(categories),
...
def get_train_transforms() -> t.Optional[tfm.Transform]:
    categories = ('images', 'dose', 'mask', 'oar', 'ptv')
    interpolators = ('bspline', 'bspline', 'nearest', 'nearest', 'nearest')

    # noinspection PyTypeChecker
    transforms = [
        tfm.Permute((2, 0, 1), entries=categories),
        aug.RandomRotation90(axes=(1, 2), p=0.3, entries=categories),
        RandomShift(shift=(0.08, 0.08), axis=(1, 2), p=0.8, entries=categories),
        aug.RandomMirror(axis=2, p=0.3, entries=categories),
        aug.RandomMirror(axis=1, p=0.3, entries=categories),
        RandomRotation(axis=0, p=0.2, entries=categories, interpolators=interpolators),
    ]
...
def get_dose_loader(
    ...
    selection_strategy = OnlyForegroundSelection('ptv')
...
#     loaders = get_loader(r"E:\raw-dldp-data\dose_dataset.h5",
#                          '../../split_config_dose/split_config_fold_0.json',
#                          2, 1, 1, 0,
#                          '../../indices/')
#
